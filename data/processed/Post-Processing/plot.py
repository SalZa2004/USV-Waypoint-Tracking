...
# Load CSV
df = pd.read_csv('PID_Controller\PID_DATA\log_20250420_121755.csv', parse_dates=['Time'])
mf = pd.read_csv('PID_Controller\PID_DATA\log_20250420_001021.csv', parse_dates=['Time'])

# Convert Time to seconds since the start
df['Time_seconds'] = (df['Time'] - df['Time'].iloc[0]).dt.total_seconds()
mf['Time_seconds'] = (mf['Time'] - mf['Time'].iloc[0]).dt.total_seconds()
# Convert speed from knots to m/s (optional)
df['Speed_mps'] = df['Speed (knots)'] * 0.514444
mf['Speed_mps'] = mf['Speed (knots)'] * 0.514444
# === Existing plots ===
...
# Thrust over time
plt.figure(figsize=(10,5))
sns.lineplot(x='Time_seconds', y='Forward Thrust', data=df)
...
df['Thrust_rate'] = df['Forward Thrust'].diff() / df['Time_seconds'].diff()
plt.figure(figsize=(10,5))
